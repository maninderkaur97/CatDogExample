...
def get_roadstress_dicts(img_dir):
    ...
    for idx, v in enumerate(imgs_anns.values()):
        if(v["regions"]):
            ...
            objs = []
            for anno in annos:
                annotations = anno["shape_attributes"]
                px = annotations["all_points_x"]
                py = annotations["all_points_y"]
                poly = [(x + 0.5, y + 0.5) for x, y in zip(px, py)]
                poly = [p for x in poly for p in x]
		
                id = -1
                if (anno["region_attributes"]["name"] == "dog"):
                        id = 0	
                elif (anno["region_attributes"]["name"] == "cat"):
                        id = 1		
