...
def config(args):
    ...
    cfg.DATASETS.TEST = ("catDogData_val",)
    ...
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = 2             # 2 category (roadway stress)
...
def main(args):
    ...
    for d in ["train", "val"]:
        DatasetCatalog.register(args.training_dataset + "_" + d, lambda d=d: get_roadstress_dicts("dataset/" + args.training_dataset + "/" + d))
        MetadataCatalog.get(args.training_dataset + "_" + d).set(thing_classes=["dog", "cat"])            # specify the category names
