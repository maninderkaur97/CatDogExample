...
def run_on_image(predictor, image):
	...
	vis = Visualizer(image[:, :, ::-1], metadata=MetadataCatalog.get("catDogData_val"), instance_mode=ColorMode.IMAGE)
	#vis = Visualizer(image[:, :, ::-1],
        #	metadata=MetadataCatalog.get("catDogData_val"), 
        #	scale=1.0, 
...
if __name__ == "__main__":
	...
	for d in ["train", "val"]:
		DatasetCatalog.register("roadstress_" + d, lambda d=d: get_roadstress_dicts("dataset/catDogData/" + d))
		MetadataCatalog.get("roadstress_" + d).set(thing_classes=["dog", "cat"])
		MetadataCatalog.get("roadstress_" + d).set(evaluator_type="coco")
		roadstress_metadata = MetadataCatalog.get("catDogData_val")
	...
	for img_path in glob.iglob(args.dataset + "/*.jpg"):
		...
		print(os.path.join(args.output, os.path.basename(img_path)))
